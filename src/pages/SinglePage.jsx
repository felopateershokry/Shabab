...
const SinglePage = () => {
        ...
        setStudent({
          ...
          lastVisit:
            data.visits && data.visits.length > 0
              ? getDateString(data.visits[data.visits.length - 1])
              : null,
        });
      }
  ...
  const visitedToday = student?.visits?.some((v) => getDateString(v) === today);
  ...
  const undoVisit = async () => {
    ...
    const newVisits = (student.visits || []).filter(
